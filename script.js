...
*/
const buton = document.querySelector(".sayiUret");

let birinci = new Array(4);
let ikinci = [];

for (let i = 0; i < birinci.length; i++) {
  const sayiUret = Math.floor(Math.random() * 99) + 1;
  birinci[i] = sayiUret;
}

ikinci = birinci
const birlestir = birinci.concat(ikinci);

const fotoNumaralari = birlestir;
...
function kartTiklama(olay) {
  ...
  if (acikKartImg.src === secilenKartImg.src) {
    ...
    if (puan === 4) {
      ...
      kutlama.src =
        "https://www.hareketligifler.net/data/media/492/havai-fisek-hareketli-resim-0021.gif";
